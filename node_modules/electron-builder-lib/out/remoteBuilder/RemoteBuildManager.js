...
class RemoteBuildManager {
                ...
                stream.on("response", function (headers) {
                    try {
                        ...
                        if (!checkStatus(status, reject)) {
                            (0, (_builderUtil || _load_builderUtil()).warn)(`Not critical server error: ${status}`);
                        }
                    } finally {
                        resolve();
                    }
            ...
            stream.on("response", headers => {
                ...
                const eventSource = new (_JsonStreamParser || _load_JsonStreamParser()).JsonStreamParser(data => {
                    ...
                    if (data.state != null) {
                        ...
                        (0, (_builderUtil || _load_builderUtil()).log)(message);
                        return;
                    }
                    if (!("files" in data)) {
                        (0, (_builderUtil || _load_builderUtil()).warn)(`Unknown builder event: ${JSON.stringify(data)}`);
                        return;
                    }
                    ...
                    for (const artifact of this.files) {
                        (0, (_builderUtil || _load_builderUtil()).log)(`Downloading ${artifact.file}`);
    ...
    downloadFile(id, artifact, resolve, reject) {
        const downloadTimer = new (_timer || _load_timer()).DevTimer("compress and upload");
        ...
        const fileUrlPath = `/v1/download${new (_url || _load_url()).URL(`f:/${id}/${artifact.file}`).pathname}`;
        const fileWritten = () => {
            this.finishedStreamCount++;
            (0, (_builderUtil || _load_builderUtil()).log)(`${artifact.file} is downloaded in ${downloadTimer.endAndGet()}`);
            if ((_builderUtil || _load_builderUtil()).debug.enabled) {
        ...
        if (!isShort) {
            ...
            (0, (_tools || _load_tools()).getAria)().then(aria2c => {
    ...
    uploadUnpackedAppArchive(stream, zstdCompressionLevel, reject) {
        ...
        (_bluebirdLst2 || _load_bluebirdLst2()).default.all([this.projectInfoManager.infoFile.value, (0, (_tools || _load_tools()).getZstd)()]).then(results => {
            const infoFile = results[0];
            (0, (_builderUtil || _load_builderUtil()).log)(`Compressing and uploading to remote build agent`);
            ...
            const zstdProcess = (0, (_child_process || _load_child_process()).spawn)(results[1], [`-${zstdCompressionLevel}`, "--long"], {
                stdio: ["pipe", "pipe", process.stderr]
            });
            ...
            zstdProcess.stdout.on("end", () => {
                (0, (_builderUtil || _load_builderUtil()).log)(`Uploaded in ${compressAndUploadTimer.endAndGet()}`);
            });
        }).catch(reject);
    }
}
